...
class Maze(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        if options is not None:
            is_evaluation = options.get("is_evaluation", is_evaluation)
        ...
        maze_pool = self.evaluation_mazes if is_evaluation else self.training_mazes
        if len(maze_pool) == 0:
            split_name = "evaluation" if is_evaluation else "training"
